...
function rosterRow(c,s){
  ...
  return `<div class="rosterRow"><div><div class="rowTitle">${esc(s.rank?`${s.rank} `:'')}${esc(s.name)}</div><div class="rowSub">${esc(s.trainingId||'No participant ID')} · ${esc(studyArmLabel(s.studyArm||c.studyArmDefault||'unassigned'))} · <span class="statusPill ${clsx}">${status}</span>${best?` · ${scoreStats(c.tierSnapshot,best).percentText}`:''}</div></div><div class="rowActions">${buttons}</div></div>`;
}
...
function addStudentForm(){
  ...
  $('formModalBody').innerHTML=`<form id="studentForm"><div class="formGrid"><label><span>Participant label / initials *</span><input name="name" required></label><label><span>Rank</span><input name="rank"></label><label><span>Participant ID *</span><input name="trainingId" required></label><label><span>Study arm</span><select name="studyArm"><option value="control" ${defaultArm==='control'?'selected':''}>Control</option><option value="frequency" ${defaultArm==='frequency'?'selected':''}>Frequency-Based</option><option value="deliberate" ${defaultArm==='deliberate'?'selected':''}>Deliberate Practice</option><option value="unassigned" ${defaultArm==='mixed'?'selected':''}>Unassigned / blinded code</option></select></label></div><div class="formActions"><button class="action primary" type="submit">Add Participant</button></div></form>`;
  $('studentForm').onsubmit=e=>{e.preventDefault();const f=new FormData(e.currentTarget),c=cls();c.students.push({id:uuid(),name:String(f.get('name')).trim(),rank:String(f.get('rank')||'').trim(),trainingId:String(f.get('trainingId')||'').trim(),studyArm:String(f.get('studyArm')||'unassigned'),attempts:{}});saveDb();closeModal('formModal');renderClass();};openModal('formModal');
}
...
function buildIndividualSummaryPdf(){
  ...
  opRect(ops,0,710,612,82,'#0B2A48');opText(ops,30,758,'FIELDREADY STUDENT PERFORMANCE SUMMARY',18,true,'#FFFFFF');opText(ops,30,738,`${s.rank?s.rank+' ':''}${s.name} | ${t.shortName} | Attempt ${st.attemptNo}`,11,true,'#CFFAFE');ensureLocationFields(c);opText(ops,30,721,`${c.name} | ${c.studyTimepoint||'baseline'} | ${studyArmLabel(s.studyArm||c.studyArmDefault||'unassigned')} | ${c.scenario||'Scenario'} v${c.scenarioVersion||'1'}`,8,false,'#E2E8F0');
...
function importRoster(file){
  ...
  r.onload=()=>{
    ...
    lines.slice(1).forEach((line,rowIndex)=>{
      ...
      const trainingId=(obj.participant_id||obj.study_id||obj.training_id||obj.id||obj.trainingid||obj.roster_id||'').trim();const studyArm=(obj.study_arm||obj.group||obj.arm||c.studyArmDefault||'unassigned').trim().toLowerCase();
      ...
      valid.push({id:uuid(),name,rank,trainingId,studyArm:['control','frequency','deliberate','unassigned'].includes(studyArm)?studyArm:(c.studyArmDefault||'unassigned'),attempts:{}});
    });
